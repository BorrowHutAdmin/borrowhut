...
public class ProductListingSeviceImplTest {
	...
	@Test
	public void testShouldgetProductListingByPartyid() throws PartyNotFoundException
	{
		ListedProductFeature listedPrdfeature = mock(ListedProductFeature.class);
	...
	prdlist.setPtyId(1);
	prdlist.setListedProductFeatures(li);	
		List<ProductListing> prdlisting=new ArrayList<ProductListing>();
		prdlisting.add(prdlist);		
		when(prdlist.getListedProductFeatures()).thenReturn(li);
		when(prdlist.getParty()).thenReturn(party);	
		when(prdlist.getProduct()).thenReturn(prd);	
		when(prd.getCategory()).thenReturn(category);
		when(productListingRepository.findByptyId(1)).thenReturn(prdlisting);
		JSONArray pllist = productListingService.getProductListingByPartyid(1);
		assertNotNull(pllist);
		verify(productListingRepository,times(1)).findByptyId(1);
