...
public class CustomDispalyedUiCardRepository {
	...
	public float gettDistanceByUicId(int uicid) {
		...
		List distList = jdbcTemplate.queryForList("SELECT TOKEN_VALUE FROM DISPLAYED_UI_CARDS WHERE TOKEN_NAME='DISTANCE' AND UIC_ID="+ uicid);
		...
		if(distList!=null && distList.size()>0)
		{
			...
			{				
				...
				distance = Float.parseFloat(scalerValue.get("TOKEN_VALUE").toString());
	...
	public boolean checkPersonalisedcardForParty(int partyId,int uicId)
	{
		...
		List uiCardList = jdbcTemplate
				.queryForList("SELECT * FROM PERSONALISED_UI_CARDS WHERE PTY_ID="+partyId+" AND  UIC_ID="+ uicId +" AND CURDATE() BETWEEN PUC_VALIDFROM AND PUC_EXPIRED" );
	...
	{
		String qry=" SELECT TOKEN_NAME,TOKEN_VALUE FROM DISPLAYED_UI_CARDS AS DIC WHERE CARD_FACE='FRONT' AND UIC_ID="+uicId+" AND TOKEN_VALUE NOT IN ('CALCULATED') ";
	...
	return	jdbcTemplate.queryForList("SELECT TOKEN_NAME,TOKEN_VALUE FROM DISPLAYED_UI_CARDS WHERE UIC_ID="+ucid+ " AND CARD_FACE='"+cardface+"' AND TOKEN_VALUE NOT IN ('CALCULATED')");
	} 
	...
	public String getDynamicColumn(int ucid) {
		...
		Map<String, Object> scalerValue;
		List dynamicColList = jdbcTemplate.queryForList("SELECT TOKEN_VALUE FROM DISPLAYED_UI_CARDS WHERE TOKEN_NAME='DYNAMIC_COLUMN' AND UIC_ID="+ ucid);
